...
/**
 * @author logosty(ganyingle) on 2020/12/11 10:46
 * 649. Dota2 参议院 中等
...
public class Solution649 {
  ...
  public String predictPartyVictory(String senate) {
    ...
    int surviveR = 0;
    int surviveD = 0;
    int[] last = new int[senate.length()];
    ...
    while (surviveD != 0 && surviveR != 0) {
      for (int i = 0; i < last.length; i++) {
        if (killR >= surviveR) {
          return "Dire";
        } else if (killD >= surviveD) {
          return "Radiant";
        }
        if (last[i] == 0) {
          continue;
    ...
    return surviveD > 0 ? "Dire" : "Radiant";
  }
